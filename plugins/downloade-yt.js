var {
  ...
  if (!text) throw 'මාතෘකාව ඇතුළත් කරන්න'
  try {
    var vid = (await youtubeSearch(text)).video[0]
    if (!vid) throw 'Video/Audio හමු වුණේ නැහැ '
    var { title, 
          description, 
          thumbnail, 
          videoId, 
  ...
  ∘ Description: ${description}`
    ...
    throw 'Video/Audio හමු වුණේ නැහැ'
  }
}
...
async function cut(url) {
  ...
  let res = await fetch(`https://api.botcahx.live/api/linkshort/bitly?link=${url}&apikey=QaepQXxR`)
