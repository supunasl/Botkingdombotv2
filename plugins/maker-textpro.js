...
let handler = async (m, { conn, args: [effect], text: txt, usedPrefix, command }) => {
  if (!effect) throw 'මෙහෙම යූස් කරන්න 😒 .textpro <එෆෙක්ට් එක>\n*උදාහරන:*\n.textpro 3Dscifi bot\n\n╭─❑ 「 LIST EFFECT 」 ❑──\n' + effects.map(v => v.title).join('\n│ • ')
  effect = effect.toLowerCase()
  if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `Efek *${effect}*දෝශයකි`
  let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
  if (text.includes(split)) text = text.split(split)
  text = Array.isArray(text) ? text : [text]
  let res = await textpro(effect, ...text)
  if (typeof res == 'number') throw res == -1 ? `එෆෙක්ට් එක *${effect}*හොයාගන්න බැ` : `ආකෘති භාවිතා කරන්න ${usedPrefix}${command} ${effect} ${new Array(res).fill('text').map((v, i) => v + (i ? i + 1 : '')).join('|')}`
